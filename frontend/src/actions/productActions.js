...
export const listProducts = ({pageNumber ='',seller = '', name='', category = '', min=0, max=0, rating = 0, order=''}) => async (dispatch) => {
 ...
 try {
     const { data } = await Axios.get(`/api/products?pageNumber=${pageNumber}&seller=${seller}&name=${name}&category=${category}&min=${min}&max=${max}&rating=${rating}&order=${order}`); // returns an array
     dispatch({type: PRODUCT_LIST_SUCCESS, payload: data}); // payload data from backend
...
export const listProductCategories = () => async (dispatch) => {
    ...
    try {
        const { data } = await Axios.get(`/api/products/categories`); // returns an array
        dispatch({type: PRODUCT_CATEGORY_LIST_SUCCESS, payload: data}); // payload data from backend
...
export const detailsProduct = (productId) => async(dispatch) => {
    ...
    try{
        const {data} = await Axios.get(`/api/products/${productId}`); // returns an object
...
export const creaeteProduct = () => async (dispatch, getState) => {
